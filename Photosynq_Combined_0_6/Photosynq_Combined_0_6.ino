...
// measuring light #2 - 520 (does not do 510 and 535 - only 520), 8,2
namespace dirk {                                 // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                      // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          measuringlight2; // 520 blue
int act_light =           actiniclight1;   // any
int red_light =           calibratinglight1;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
int detector =            detector2;
int pulsesize =           20;                         // measured in microseconds
int actintensity1 =       20;                         // intensity at LOW setting below
int actintensity2 =       255;                        // intensity at HIGH setting below
int measintensity =       255;                        // 255 is max intensity during pulses, 0 is minimum // for additional adjustment, change resistor values on the board
int pulses [] =           {100,100,100};              // Maximum pulses per cycle 100
int pulsedistance [][1] = {{1000},{1000},{1000}};     // measured in us. Minimum 200us
int act [] =              {2,HIGH,2};                // 2 is off.  "LOW" is actintensity1, "HIGH" is actintensity2.  May use this for combined actinic / saturation pulses  .  NOTE! You may set the intensity at any value during the run even if it's not preset - however, the LED intensity rise time is a few milliseconds.  The rise time on the preset values is in the nanoseconds range.
int alt1 [] =             {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int alt2 [] =             {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int red [] =              {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int total_cycles = sizeof(pulses)/sizeof(pulses[0])-1;// (start counting at 0!)
}

// measuring light #2 - 520 (does not do 510 and 535 - only 520), 8,2
namespace dirk2 {                                 // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                      // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        8;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          measuringlight2; // 520 blue
int act_light =           actiniclight1;   // any
int red_light =           calibratinglight1;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
int detector =            detector2;
int pulsesize =           50;                         // measured in microseconds
int actintensity1 =       20;                         // intensity at LOW setting below
int actintensity2 =       255;                        // intensity at HIGH setting below
int measintensity =       255;                        // 255 is max intensity during pulses, 0 is minimum // for additional adjustment, change resistor values on the board
int pulses [] =           {100,100,100};              // Maximum pulses per cycle 100
...
// measuring light 4 // wait 10, repeats 5
namespace fluorescence {                              // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                         // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          measuringlight1; // orange
int act_light =           actiniclight1;   // any
int red_light =           calibratinglight1;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
int detector =            detector1;
int pulsesize =           20;                         // measured in microseconds
int actintensity1 =       30;                         // intensity at LOW setting below
int actintensity2 =       255;                        // intensity at HIGH setting below
int measintensity =       255;                        // 255 is max intensity during pulses, 0 is minimum // for additional adjustment, change resistor values on the board
int pulses [] =           {50,50,50,50};              // Maximum pulses per cycle 100
int pulsedistance [][1] = {{10000},{10000},{10000},{10000}};     // measured in us. Minimum 200us
int act [] =              {2,HIGH,2,2};                // 2 is off.  "LOW" is actintensity1, "HIGH" is actintensity2.  May use this for combined actinic / saturation pulses  .  NOTE! You may set the intensity at any value during the run even if it's not preset - however, the LED intensity rise time is a few milliseconds.  The rise time on the preset values is in the nanoseconds range.
int alt1 [] =             {LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int alt2 [] =             {LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int red [] =              {LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int total_cycles = sizeof(pulses)/sizeof(pulses[0])-1;// (start counting at 0!)
}

// measuring light 4 // wait 10, repeats 5
namespace spad {                              // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                         // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          measuringlight1; // 940 
int act_light =           actiniclight1;   // 650 far red
int red_light =           calibratinglight1;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
int detector =            detector1;
...
int actintensity2 =       255;                        // intensity at HIGH setting below
int measintensity =       255;                        // 255 is max intensity during pulses, 0 is minimum // for additional adjustment, change resistor values on the board
int pulses [] =           {50,1,50,50,50};            // Maximum pulses per cycle 100
int pulsedistance [][1] = {{10000},{10000},{10000},{10000},{10000}};     // measured in us. Minimum 200us
int act [] =              {2,HIGH,2,2,2};                // 2 is off.  "LOW" is actintensity1, "HIGH" is actintensity2.  May use this for combined actinic / saturation pulses  .  NOTE! You may set the intensity at any value during the run even if it's not preset - however, the LED intensity rise time is a few milliseconds.  The rise time on the preset values is in the nanoseconds range.
int alt1 [] =             {LOW,LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int alt2 [] =             {LOW,LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int red [] =              {LOW,LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int total_cycles = sizeof(pulses)/sizeof(pulses[0])-1;// (start counting at 0!)
}

// measuring light 4 // wait 10, repeats 5
namespace fluorescence2 {                              // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                         // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        8;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
...
// 3 measuring lights! currently unused in protocol
namespace parsing {                                 // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                2;                      // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          measuringlight1; // 520 blue
int act_light =           actiniclight1;   // any
int red_light =           measuringlight2;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
int detector =            detector1;
int pulsesize =           25;                         // measured in microseconds
int actintensity1 =       30;                         // intensity at LOW setting below
int actintensity2 =       255;                        // intensity at HIGH setting below
int measintensity =       255;                        // 255 is max intensity during pulses, 0 is minimum // for additional adjustment, change resistor values on the board
int pulses [] =           {300,300,300};              // Maximum pulses per cycle 100
int pulsedistance [][1] = {{1000},{1000},{1000}};     // measured in us. Minimum 200us
int act [] =              {LOW,2,LOW};                // 2 is off.  "LOW" is actintensity1, "HIGH" is actintensity2.  May use this for combined actinic / saturation pulses  .  NOTE! You may set the intensity at any value during the run even if it's not preset - however, the LED intensity rise time is a few milliseconds.  The rise time on the preset values is in the nanoseconds range.
int alt1 [] =             {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int alt2 [] =             {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int red [] =              {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int total_cycles = sizeof(pulses)/sizeof(pulses[0])-1;// (start counting at 0!)
...
// currently unusued
namespace ECScal {                                 // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                2;                      // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
...
// measuring light 5 // 16, 15
namespace eight_ten_dirk {                                 // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                         // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          measuringlight3; // 810nm
int act_light =           actiniclight2;   // any
int red_light =           calibratinglight2;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
int detector =            detector1;
int pulsesize =           25;                         // measured in microseconds
...
// uses 810 and 940 for measuring pulse // 8, 20
namespace eight_ten_saturation {                           // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                      // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          measuringlight4; // 520 blue
int act_light =           actiniclight2;   // any
int red_light =           calibratinglight2;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
...
int actintensity2 =       255;                        // intensity at HIGH setting below
int measintensity =       255;                        // 255 is max intensity during pulses, 0 is minimum // for additional adjustment, change resistor values on the board
int pulses [] =           {100,100,500,500,200,100};              // Maximum pulses per cycle 100
int pulsedistance [][1] = {{3000},{3000},{3000},{3000},{3000},{3000}};     // measured in us. Minimum 200us
int act [] =              {LOW,HIGH,2,2,HIGH,LOW};                // 2 is off.  "LOW" is actintensity1, "HIGH" is actintensity2.  May use this for combined actinic / saturation pulses  .  NOTE! You may set the intensity at any value during the run even if it's not preset - however, the LED intensity rise time is a few milliseconds.  The rise time on the preset values is in the nanoseconds range.
int alt1 [] =             {LOW,LOW,LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int alt2 [] =             {LOW,LOW,LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int red [] =              {LOW,LOW,LOW,LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int total_cycles = sizeof(pulses)/sizeof(pulses[0])-1;// (start counting at 0!)
}

// measuring light #2 - 520 (does not do 510 and 535 - only 520), 8,2
namespace calibration2 {                                 // Variables for PS1 measurement using 850nm and actinic light
int repeats =             1;                         // number of times to repeat the entire run (so if averages = 6 and repeats = 3, total runs = 18, total outputted finished data = 3)
int wait =                1;                      // seconds wait time between repeats
int averages =            1;                          // number of runs to average
int measurements =        3;                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
int meas_light =          calibratinglight2; // 940
int act_light =           actiniclight1;   // any
int red_light =           calibratinglight1;
int alt1_light =          measuringlight2;
int alt2_light =          measuringlight2;
int detector =            detector2;
int pulsesize =           25;                         // measured in microseconds
int actintensity1 =       255;                         // intensity at LOW setting below
int actintensity2 =       255;                        // intensity at HIGH setting below
int measintensity =       255;                        // 255 is max intensity during pulses, 0 is minimum // for additional adjustment, change resistor values on the board
int pulses [] =           {300,300,300};              // Maximum pulses per cycle 100
int pulsedistance [][1] = {{10000},{10000},{10000}};     // measured in us. Minimum 200us
int act [] =              {2,LOW,2};                // 2 is off.  "LOW" is actintensity1, "HIGH" is actintensity2.  May use this for combined actinic / saturation pulses  .  NOTE! You may set the intensity at any value during the run even if it's not preset - however, the LED intensity rise time is a few milliseconds.  The rise time on the preset values is in the nanoseconds range.
int alt1 [] =             {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int alt2 [] =             {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int red [] =              {LOW,LOW,LOW};              // If set to measuring or calibrating light, "HIGH" is on and "LOW" is off. If set to actinic light, LOW is actintensity1, HIGH is actintensity2. 
int total_cycles = sizeof(pulses)/sizeof(pulses[0])-1;// (start counting at 0!)
...
Serial.println(sizeof(pulses[0]));
  ...
  case 000:        // CALIBRATION
    ...
    break;
  case 001:        // ps1
    fluorescence_protocol_main();                     //fofm
    delay(30000);
    fluorescence2_protocol_main();                     //fofm
    delay(30000);
    dirk_protocol_main();                     //fofm
    delay(30000);    
    dirk2_protocol_main();                     //fofm
    delay(30000);        
    fluorescence_protocol_main();                     //fofm
    delay(30000);
    fluorescence2_protocol_main();                     //fofm
...
/*
    ...
    fluorescence_protocol_main();                     //fofm
    countdown(30);
    ets_protocol_main();                              //phi1
    countdown(10);    
    etd_protocol_main();                              //dirk_p700
    ets_protocol_main();                              //phi1
    ...
    wait = 120;
    act [] = {LOW,HIGH,2,LOW};
    fluorescence_protocol_main();                     //flind 
    countdown(60);
    dirk_protocol_main();                             //dirkECS
    etd_protocol_main();                              //dirk_p700
    ets_protocol_main();                              //phi1
    countdown(10);
    act [] = {LOW,HIGH,2,LOW};
    fluorescence_protocol_main();                     //fsfmp
    act [] = {2,HIGH,2,2};
    repeats = 6;
    wait = 90;
    fluorescence_protocol_main();                     //darkrec
    countdown(10);
    ets_protocol_main();                             //phi1
    Serial.print("Round ");
    Serial.print(j);
...
using namespace spad;
  for (x=0;x<repeats;x++) {                                                                        // Repeat the entire measurement this many times  
    ...
    for (y=0;y<averages;y++) {                                                                     // Average this many measurements together to yield a single measurement output
      while ((cycle <= total_cycles) && (pulse <= pulses[total_cycles])) {                      // Keep doing the following until the last pulse of the last cycle...
...
//          calibrationsample();                                                                         // Run calibration
          ...
          analogWrite(meas_light, measintensity);
          digitalWriteFast(meas_light, LOW);                                                        // make sure the measuring light doesn't flash at the beginning
          ...
          #endif
          starttimer0 = micros();
          timer0.begin(spad_pulse1,pulsedistance[cycle][0]);                                       // Begin firsts pulse
          while (micros()-starttimer0<pulsesize) {}                                               // wait a full pulse size, then...                                                                                          
          timer1.begin(spad_pulse2,pulsedistance[cycle][0]);                                       // Begin second pulse
          #ifdef DEBUG
        ...
        if ((cycle < total_cycles) | (pulse < pulses[total_cycles])) {                                        // Add a comma between lists if it's not the last measurement
          Serial.print(",");      
          Serial1.print(",");
        }     
    ...
    timer1.end();
    digitalWriteFast(meas_light, LOW);                                                      // ..make sure remaining lights are off
...
//          calibrationsample();                                                                         // Run calibration
          ...
          #endif
          starttimer0 = micros();
          timer0.begin(dirk_pulse1,pulsedistance[cycle][0]);                                       // Begin firsts pulse
          while (micros()-starttimer0<pulsesize) {}                                               // wait a full pulse size, then...                                                                                          
          timer1.begin(dirk_pulse2,pulsedistance[cycle][0]);                                       // Begin second pulse
          #ifdef DEBUG
          Serial.println(starttimer0);
          #endif
        }  
...
int dirk2_protocol_main() {
using namespace dirk2;
  for (x=0;x<repeats;x++) {                                                                        // Repeat the entire measurement this many times  
    Serial1.print("{\"device_version\": 1,");                                                    //Begin JSON file printed to bluetooth on Serial ports
    Serial.print("{\"device_version\": 1,");
    Serial1.print("\"protocol\": ");
...
int fluorescence_protocol_main() {
using namespace fluorescence;
  for (x=0;x<repeats;x++) {                                                                        // Repeat the entire measurement this many times  
    Serial1.print("{\"device_version\": 1,");                                                    //Begin JSON file printed to bluetooth on Serial ports
    Serial.print("{\"device_version\": 1,");
    Serial1.print("\"protocol\": ");
    Serial.print("\"protocol\": ");
    Serial1.print("\"fluorescence\",");
...
//          calibrationsample();                                                                         // Run calibration
          ...
          #endif
          starttimer0 = micros();
          timer0.begin(fluorescence_pulse1,pulsedistance[cycle][0]);                                       // Begin firsts pulse
          while (micros()-starttimer0<pulsesize) {}                                               // wait a full pulse size, then...                                                                                          
          timer1.begin(fluorescence_pulse2,pulsedistance[cycle][0]);                                       // Begin second pulse
          #ifdef DEBUG
          Serial.println(starttimer0);
          #endif
        }
...
int fluorescence2_protocol_main() {
using namespace fluorescence2;
  for (x=0;x<repeats;x++) {                                                                        // Repeat the entire measurement this many times  
    Serial1.print("{\"device_version\": 1,");                                                    //Begin JSON file printed to bluetooth on Serial ports
    Serial.print("{\"device_version\": 1,");
    Serial1.print("\"protocol\": ");
...
int etd_protocol_main() {
using namespace eight_ten_dirk;
  for (x=0;x<repeats;x++) {                                                                        // Repeat the entire measurement this many times  
    Serial1.print("{\"device_version\": 1,");                                                    //Begin JSON file printed to bluetooth on Serial ports
    Serial.print("{\"device_version\": 1,");
    Serial1.print("\"protocol\": ");
    Serial.print("\"protocol\": ");
    Serial1.print("\"810 dirk\",");
    Serial.print("\"810 dirk\",");    
    for (y=0;y<averages;y++) {                                                                     // Average this many measurements together to yield a single measurement output
      while ((cycle <= total_cycles) && (pulse <= pulses[total_cycles])) {                      // Keep doing the following until the last pulse of the last cycle...
        if (cycle == 0 && pulse == 0) {                                                                // if it's the beginning of a measurement, then...
          Serial1.print("\"average\": ");                                                                // Start the beginning of this variable in JSON format
...
//          calibrationsample();                                                                         // Run calibration
          ...
          #endif
          starttimer0 = micros();
          timer0.begin(etd_pulse1,pulsedistance[cycle][0]);                                       // Begin firsts pulse
          while (micros()-starttimer0<pulsesize) {}                                               // wait a full pulse size, then...                                                                                          
          timer1.begin(etd_pulse2,pulsedistance[cycle][0]);                                       // Begin second pulse
          #ifdef DEBUG
          Serial.println(starttimer0);
          #endif
        }  
...
int ets_protocol_main() {
using namespace eight_ten_saturation;
  for (x=0;x<repeats;x++) {                                                                        // Repeat the entire measurement this many times  
    Serial1.print("{\"device_version\": 1,");                                                    //Begin JSON file printed to bluetooth on Serial ports
    Serial.print("{\"device_version\": 1,");
    Serial1.print("\"protocol\": ");
    Serial.print("\"protocol\": ");
    Serial1.print("\"810 sat\",");
    Serial.print("\"810 sat\",");    
    for (y=0;y<averages;y++) {                                                                     // Average this many measurements together to yield a single measurement output
      while ((cycle <= total_cycles) && (pulse <= pulses[total_cycles])) {                      // Keep doing the following until the last pulse of the last cycle...
        if (cycle == 0 && pulse == 0) {                                                                // if it's the beginning of a measurement, then...
          Serial1.print("\"average\": ");                                                                // Start the beginning of this variable in JSON format
...
//          calibrationsample();                                                                         // Run calibration
          ...
          #endif
          starttimer0 = micros();
          timer0.begin(ets_pulse1,pulsedistance[cycle][0]);                                       // Begin firsts pulse
          while (micros()-starttimer0<pulsesize) {}                                               // wait a full pulse size, then...                                                                                          
          timer1.begin(ets_pulse2,pulsedistance[cycle][0]);                                       // Begin second pulse
          #ifdef DEBUG
          Serial.println(starttimer0);
          #endif
        }  
