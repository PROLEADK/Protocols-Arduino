/*
DESCRIPTION:
  DIRK-F stands for Dark Inra-Red Response Kinetics.  In this method, the measuring light is always on, with a 10 us off period occuring every 1 ms, repeated 10 times total.
  The primary outputs are the slope of the initial drop in fluoresence during the OFF cycle (called "slope"), the average of the values during the ON cycle just prior
  ... to the off cycle ("data1" through "data6" then averaged as "average"), and each of the individual detector values measured during the OFF cycle itself "called data7, data8, data9" averaged
  ... over all of the off cycles throughout the entire run.
...
// Max cycles is based on the saved array size 

// NOTES:
// Shortest period for running 6 averaged analogReads = 5 sec per analog read.
// At best, in 10us I can only get ...?
// jitter mostly comes from file.print commands to the SD card... so having many of these is a bad idea in this case
//... then check to see if the baseline values make sense - they seem a bit fishy, don't they?
// ---> Jitter is much reduced using the array, however, the array limits the number of cycles to ~200 before needing to reset the array or create a new array
// The total sample time is abotu 949us over what it should be (that's about 20us per cycle)
// Also, the analogread()s are a bit far apart, and it's hard to identify the slope of the decay curve - any way to smuch these together
...
int pulsecycles = 50; // Number of times the "pulselengthon" and "pulselengthoff" cycle (on/off is 1 cycle) (maximum = 1000 until we can store the read data in an SD card or address the interrupts issue)
...
unsigned long pulselengthon = 990; // Pulse LED on length in uS (minimum = 5us based on a single ~4us analogRead - +5us for each additional analogRead measurement in the pulse).
unsigned long pulselengthoff = 10; // Pulse LED off length in uS (minimum = 20us + any additional operations which you may want to call during that time).
...
int detector1 = A1; // Teensy analog pin for detector

// INTERNAL VARIABLES, COUNTERS, ETC.
unsigned long start1,start1orig,end1, calstart1orig, calend1, start2, start3, start5, start6, start7, end5;
...
void dirkf() {
  ...
  strcat(filenamelocal, "-U.CSV"); // NOTE! "filename" + calibration name extension (CAN ONLY BE A DASH (-) AND 1 LETTER!).  This is the name of file that this subroutine data is stored in
  SdFile sub1;
  ...
  datasample = (int*)malloc(pulsecycles*9*sizeof(int)); // NOTE! multiply by the number of total saved data samples per cycle.  Create the array of proper size to save one value for all each ON/OFF cycle
  noInterrupts();
  
  int analogreads = 10;

  file.println(micros()); // first row shows the run time in milliseconds (start of program = 0 ms)
  start1orig = micros();
  start1 = micros();
  for (i=0;i<pulsecycles;i++) {
    digitalWriteFast(measuringlight1, HIGH); 
    data6 = analogRead(detector1);
    data7 = analogRead(detector1);
    data8 = analogRead(detector1);
    data9 = analogRead(detector1);
    start1=start1+pulselengthon;
    start2=start1+pulselengthon - 45; // Allow 45 additional us just before before the OFF cycle for the analogReads  
    start3=start1+pulselengthoff;    

    int localflag = i*analogreads; // NOTE! multiply by the number of total saved data samples per cycle.
    datasample[localflag] = data0-baseline;
    datasample[1+localflag] = data1-baseline;
    datasample[2+localflag] = data2-baseline;
    datasample[3+localflag] = data3-baseline;
    datasample[4+localflag] = data4-baseline;
    datasample[5+localflag] = data5-baseline;
    datasample[6+localflag] = data6-baseline;
    datasample[7+localflag] = data7-baseline;
    datasample[8+localflag] = data8-baseline;
    datasample[9+localflag] = data9-baseline;
    ...
    digitalWriteFast(measuringlight1, LOW);
    data4 = analogRead(detector1); // maximum of 2 analog reads in 10us
//    data5 = analogRead(detector1);
//    data7 = analogRead(detector1);
    while (micros()<start3) {} 
  }
  end1 = micros();
...
Serial.println("Sample data from the array!");
for (i=0;i<pulsecycles*analogreads; i++) { // NOTE! change the number here based on the number of data samples per cycle!
  Serial.print(datasample[i]);
  Serial.print(",");
  if (i%(analogreads) == (analogreads-1)) {
    Serial.println("");
  }
}
