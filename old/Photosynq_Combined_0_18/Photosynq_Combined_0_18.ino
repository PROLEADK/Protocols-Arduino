...
//    delay(1000);
  ...
  for (y=0;y<measurements;y++) {                                                      // loop through the all measurements to create a measurement group
    ...
    for (int q = 0;q<number_of_protocols;q++) {                                               // loop through all of the protocols to create a measurement
      ...
      int act_light =           hashTable.getLong("act_light");                               // the Teensy pin # to associate with the actinic light  Must be pin 2, 20 (ie actinic pin only)
...
//        digitalWriteFast(act_background_light,LOW);                                          // turn off actinic background from previous protocol
      ...
      int actintensity1 =       hashTable.getLong("actintensity1");                         // actinic light preset intensity 1.  0 is off, 20 is 2000 uE.  Typical saturating pulse is 50.  This value is used automatically when applying a background actinic light.
      int actintensity2 =       hashTable.getLong("actintensity2");                        // actinic light preset intensity 2.  0 is off, 20 is 2000 uE.  Typical saturating pulse is 50.
      int measintensity =       hashTable.getLong("measintensity");                        // measuring light intensity.  0 is maximum, 100 is minimum.   for additional adjustment, change resistor values on the board
      int calintensity =        hashTable.getLong("calintensity");                        // calibrating light intensity.  255 is maximum, 0 is minimum.   for additional adjustment, change resistor values on the board
...
// NOTE: for act, act2, alt1, and alt2, you can choose a measuring light.  If you do there are no preset states.  Select 2 for off, or 1 for on.
      JsonArray act =           hashTable.getArray("act");                                // the state (0:preset 1, 1:preset 2, 2:off) of the actinic light in an array.  For example, [0,2,2] is actinic light on at preset 1, actinic light on at preset 2, actinic light off
    ...
    options for relative humidity, temperature, contactless temperature. light_intensity,co2
      ...
      if (tcs_to_act > 0) {                                                           // if you're setting actinic intensity based on outside light as per the tcs light sensor.  The deep red actinic produces 103 uE per pwm step.
        actintensity1 = (lux_to_uE(lux_average)*tcs_to_act/(103*100));
        actfull = (lux_to_uE(lux_average)/(103));                                     // just to confirm that actintensity is in fact a fraction of lux_to_uE if tcs_to_act is not 100%.
      }
