...
// still locks up frequently, somehow related to the first measurement pulse after the baseline is statement
// need to switch to IntervalTimer from PITimer for dirkf
// while it does use a timer for dirkf, it also uses the while() method, which isn't quite as accurate and does increase jitter, especially on the back end of the signal
...
int ddelayruns = 5000; // millisecond delay between each repeated run
...
volatile float dcyclelength = .01; // time between cycles of pulse/actinicoff/pulse/actinic on in seconds... minimum = pulselengthon + 7.5us
...
volatile int data1=0, data2=0, data3=0, data4=0, data5=0, data6=0, data7=0, data8=0, data9=0, data10=0, caldata1, caldata2, caldata3, caldata4, analogresolutionvalue;
...
/*
// USE FOR SETTING MANUAL TIME (hr,min,sec,mo,day,yr)

  Serial.println("Please write the current day using 2 digits");
...
//  int hours=0, minutes=0, seconds=0, days=0, months=0, years=0;
  ...
  while (Serial.available()>2 && Serial.available()<5) {}
    

  while (Serial.available()>0 && Serial.available()<3) {}
        strcat(hours, (char)Serial.read());
...
/*
  ...
  while (Serial.available()>6 && Serial.available()<9) {}
    months = months*10;
    months = months + Serial.read();

  while (Serial.available()>4 && Serial.available()<7) {
    months = months*10;
    months = months + Serial.read();
  } 
  while (Serial.available()>0 && Serial.available()<5) {
    years = years*10;
    years = years + Serial.read();
  }

  Serial.println("Please write the current the hour (ie 14), min (ie 56), and sec (ie 32) and press enter");
  Serial.println("(if you make a mistake, restart teensy and reenter)");

  while (Serial.available()<6) {}
  while (Serial.available()>4 && Serial.available()<7) {
    hours = hours*10;
    hours = hours + Serial.read();
  } 
  while (Serial.available()>2 && Serial.available()<5) {
    minutes = minutes*10;
    minutes = minutes + Serial.read();
  } 
  while (Serial.available()>0 && Serial.available()<3) {
    seconds = seconds*10;
    seconds = seconds + Serial.read();
  }
...
setTime(hours, minutes, seconds, months, days, years);

*/

// USE FOR SETTING ASCII TIME
...
void dpulse2() {
  ...
  if (dsaturatingcycleoff == PITimer1.count()) {
