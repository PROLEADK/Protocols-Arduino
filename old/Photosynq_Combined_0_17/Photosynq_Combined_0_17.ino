...
//    delay(1000);
    ...
    else if (Serial.peek() == 55 | Serial1.peek() == 55) {                      // teensy power down on pin 22
  ...
  for (y=0;y<measurements;y++) {                                                      // loop through the all measurements to create a measurement group
    ...
    for (int q = 0;q<number_of_protocols;q++) {                                               // loop through all of the protocols to create a measurement
      ...
      int protocols_delay =     hashTable.getLong("protocols_delay");
...
//        Serial.println(hashTable.getLong("act_background_light"));
      ...
      int tcs_to_act =          hashTable.getLong("tcs_to_act");                         // sets the % of response from the tcs light sensor to act as actinic during the run (values 1 - 100)
        ...
        && (String) environmental.getArray(i).getString(0) == "light_intensity") {
            ...
            if (x == averages-1) {
              ...
              Serial.print(",");  
                                                                                      ...
                                                                                      // NOTE: when using tcs_to_act, set actintensity1 to zero 
      if (tcs_to_act > 0) {                                                           // if you're setting actinic intensity based on outside light as per the tcs light sensor
        actintensity1 = (lux_to_uE(lux_average)*(tcs_to_act/100))/103;
        analogWrite(ACTINICLIGHT_INTENSITY1, actintensity1);                      // set intensities for each of the lights - this is high (saturating)
        digitalWriteFast(ACTINICLIGHT_INTENSITY_SWITCH, LOW);                     // preset the switch to the actinic (low) preset position
#ifdef DEBUGSIMPLE
...
void lighttests() {
  ...
  while (choose!=999) {
    ...
    Serial.println("001 - light detector testing (press any key to exit after entering))");
    Serial.println("002 - CO2 testing (press any key to exit after entering))");
    ...
    Serial1.println("001 - light detector testing (press any key to exit after entering))");
    Serial1.println("002 - CO2 testing (press any key to exit after entering))");
...
float lux_to_uE(float _lux_average) {
  int uE = (_lux_average-996.668)/30.64773;
  return uE;
}
