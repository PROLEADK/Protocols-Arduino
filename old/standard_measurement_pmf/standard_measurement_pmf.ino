/*
DESCRIPTION:
This file is used to measure pulse modulated fluorescence (PMF) using a saturating pulse and a measuring pulse
...
int data1, data2, data3, data4, dataaverage;
...
for (i=0;i<pulsecycles;i++) {
  ...
  dataaverage = (data1+data2+data3+data4)/4; // Again, if you are using less than 4 data points, change the value that the points are divided by here.
  data += dataaverage;
  data += ",";
  while (micros()<start1) {} 
}
