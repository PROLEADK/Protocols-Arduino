...
// get serial1.print for sd card inforamtion (right now it just prints to serial.print)
...
const char* protocolname = "Photosynq Main"; // name of this arduino file (aka protocol)
...
int dnoactruns = 0; // number of runs (occuring at beginning) which do not turn actinic off (maximum = drepeatrun)
...
void dirkf() {
  ...
  for (x=0;x<drepeatrun;x++) {
    ...
    Serial1.print("Measuring Pulse 1a: with actinic pulse");
    Serial1.print(",");
    Serial1.print("Measuring Pulse 2a: with actinic pulse");
    Serial1.println(""); 
...
void dpulse1() {
  ...
  digitalWriteFast(measuringlight1, LOW);
  if (dpulse1count%2 == 0) {
    digitalWriteFast(actiniclight1, LOW); // turn actinic on during the even cycles
  data1 = data0;
  }
  else {
    data3 = data0;
...
void dpulse2() {
  ...
  if (dpulse2count%2 == 0) {
    digitalWriteFast(actiniclight1, HIGH); // turn actinic on during the even cycles
    data2 = data0;
  }    
  else {
    data4 = data0;
