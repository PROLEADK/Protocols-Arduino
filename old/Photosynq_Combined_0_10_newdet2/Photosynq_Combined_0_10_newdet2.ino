...
int detector1 = A10;                                                          // Teensy analog pin for detector 34
int detector2 = A11;                                                          // Teensy analog pin for detector 35
...
case 100:
    Serial.println("{\"device_id\": 1, \"firmware_version\": \"Alfie Arabidopsis\", \"protocol_number\": 1, \"protocol_name\": \"fluorescence\", \"baseline_sample\": 0.00,\"baseline_values\": [0.00,0.00,0.00],\"chlorophyll_spad_calibration\": [1435.00,90.83],\"light_intensity\": 0,\"red\": 1,\"green\": 0,\"blue\": 0,\"relative_humidity\": 17.97,\"temperature\": 25.05,\"co2_content\": 757,\"averages\": 0,\"repeats\": 0,\"data_raw\": [367.00,353.00,389.00,370.00,349.00,357.00,352.00,330.00,360.00,346.00,367.00,377.00,328.00,401.00,366.00,366.00,402.00,346.00,353.00,362.00,378.00,360.00,354.00,346.00,370.00,408.00,356.00,365.00,360.00,346.00,361.00,341.00,375.00,363.00,364.00,361.00,370.00,391.00,374.00,343.00,354.00,351.00,398.00,369.00,342.00,347.00,347.00,396.00,360.00,372.00,346.00,1026.00,1018.00,1063.00,1029.00,1036.00,1051.00,1033.00,1069.00,1037.00,1011.00,1080.00,1052.00,1063.00,1067.00,1063.00,1060.00,1034.00,1056.00,1063.00,1063.00,1051.00,1070.00,1012.00,1069.00,1060.00,1062.00,1091.00,1083.00,1060.00,1023.00,1057.00,1076.00,1049.00,1050.00,1057.00,1053.00,1035.00,1034.00,1034.00,1060.00,1031.00,1063.00,1039.00,1044.00,1095.00,1067.00,1051.00,1043.00,1061.00,1039.00,882.00,777.00,679.00,629.00,593.00,610.00,586.00,529.00,540.00,536.00,506.00,537.00,524.00,496.00,514.00,499.00,486.00,495.00,476.00,496.00,453.00,513.00,499.00,518.00,462.00,432.00,494.00,487.00,500.00,451.00,464.00,486.00,477.00,480.00,468.00,448.00,496.00,454.00,421.00,454.00,469.00,458.00,486.00,459.00,458.00,445.00,446.00,452.00,457.00,417.00,488.00,441.00,478.00,462.00,462.00,440.00,448.00,448.00,448.00,457.00,442.00,419.00,415.00,452.00,450.00,450.00,446.00,458.00,452.00,454.00,449.00,463.00,423.00,483.00,427.00,437.00,409.00,446.00,443.00,448.00,440.00,439.00,431.00,432.00,425.00,412.00,458.00,453.00,433.00,470.00,415.00,444.00,429.00,425.00,438.00,445.00,444.00,437.00,469.00,469.00],\"end\":1}"); 
    break;
...
int detector2 = A11;                                                          // Teensy analog pin for detector
    ...
    // {"protocol_name":"fluorescence","repeats":1,"wait":0,"averages":1,"measurements":3,"meas2_light":15,"meas1_baseline":0,"act_light":20,"pulsesize":25,"pulsedistance":10000,"actintensity1":50,"actintensity2":255,"measintensity":255,"calintensity":255,"pulses":[50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50],"act":[2,1,2,2],"red":[2,2,2,2],"alt1":[2,2,2,2],"alt2":[2,2,2,2],"measlights":[[15,15,15,15],[15,15,15,15],[15,15,15,15],[15,15,15,15]],"detectors":[[34,34,34,34],[34,34,34,34],[34,34,34,34],[34,34,34,34]]}


*/

// So the hashTable.getLong calls are all pretty slow as compared to a normal call to an array (85us verssu 5us)... so I'm going to convert all of the variables from the JSON into 
// standard c++ variables so that they can be called quicker.

  int detectors [4][4] = {{34,34,34,34},{34,34,34,34},{34,34,34,34},{34,34,34,34}};

    cal_true =            hashTable.getBool("cal_true");                             // identify this as a calibration routine (0 = false, 1 = re-calibrate, 2 = calibrate to sample)
    ...
    wait =                hashTable.getLong("wait");                         // seconds wait time between repeats
    averages =            hashTable.getLong("averages");                          // number of runs to average
    measurements =        hashTable.getLong("measurements");                          // # of measurements per pulse to be averaged (min 1 measurement per 6us pulselengthon)
    meas1_light =         hashTable.getLong("meas1_light"); // 850
    meas2_light =         hashTable.getLong("meas2_light"); // 850
    meas3_light =         hashTable.getLong("meas3_light"); // 850
    meas4_light =         hashTable.getLong("meas4_light"); // 850
...
//   int length_out = hashTable.getArray("measlights").getLength();
//   int length_in = hashTable.getArray("measlights").getArray(0).getLength();
/*
   measlights [length_out][length_in];
...
*/
    ...
    red_ =                hashTable.getArray("red");
    alt1_ =               hashTable.getArray("alt1");
    alt2_ =               hashTable.getArray("alt2");
    detectors_ =          hashTable.getArray("detectors");
    total_cycles =        sizeof(pulses)/sizeof(pulses[0])-1;        // (start counting at 0!)
        
    Serial.println(    hashTable.getArray("measlights").getLength());
    Serial.println(        hashTable.getArray("measlights").getArray(0).getLength());
        
/*
So... now we need to figure out how we can use these hashTable calls... to see if we can stick them right 
*/
...
int end20;
int start20 = micros();
//        Serial.println(hashTable.getLong("repeats"));
//        Serial.println(hashTable.getBool("cal_true"));
//        Serial.println(hashTable.getArray("pulses").getLong(0));
        Serial.println(hashTable.getArray("measlights").getArray(0).getLong(0));        
end20 = micros();
Serial.print("this took:  ");
Serial.println(end20 - start20);

start20 = micros();
        Serial.println(detectors[0][0]);        
end20 = micros();
Serial.print("this took:  ");
Serial.println(end20 - start20);
        
   Serial.println(cal_true);
   Serial.println(protocol_name);
   Serial.println(repeats);
   Serial.println(wait);
   Serial.println(averages);
   Serial.println(measurements);
   Serial.println(meas1_light);
   Serial.println(meas2_light);
   Serial.println(meas3_light);
   Serial.println(meas4_light);
   Serial.println(act_light);
   Serial.println(pulsesize);
   Serial.println(pulsedistance);
   Serial.println(actintensity1);
   Serial.println(actintensity2);
   Serial.println(measintensity);
   Serial.println(calintensity);

    Serial.println("pulses: ");
    for (int i = 0; i < pulses_.getLength(); i++)
    {
        int value = pulses_.getLong(i);
        Serial.print(i);
        Serial.print(" ");
    ...
    Serial.println("red: ");
    for (int i = 0; i < red_.getLength(); i++)
    {
        int value = red_.getLong(i);
        Serial.print(i);
        Serial.print(" ");
        Serial.println(value);
    }
    Serial.println("alt1: ");
    for (int i = 0; i < alt1_.getLength(); i++)
    {
        int value = alt1_.getLong(i);
        Serial.print(i);
        Serial.print(" ");
        Serial.println(value);
    }
    Serial.println("alt2: ");
    for (int i = 0; i < alt2_.getLength(); i++)
    {
        int value = alt2_.getLong(i);
        Serial.print(i);
        Serial.print(" ");
        Serial.println(value);
    }
    ...
    for (int i = 0; i < detectors_.getLength(); i++)
    {
        ...
        {
            int value = innerArray.getLong(j);
...
_meas_light = measlights[cycle][meas_number%4];                                    // move to next measurement light
detector = detectors[cycle][meas_number%4];                                        // move to next detector
