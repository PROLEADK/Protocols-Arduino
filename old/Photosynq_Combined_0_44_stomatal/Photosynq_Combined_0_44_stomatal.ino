...
////////////////////MLX90615 settings (01011100 (0x5B) for add on and 01011101 (0x5C) for main //////////////////////
int MLX90615_address_addon = 0x5B;  // 0x5B is default
int MLX90615_address_main = 0x5C;  // 0x5B is default
...
int clockPin_main = 10;  // pin used for clock on main board sht15
int dataPin_main  = 9;  // pin used for data on main board sht15
int clockPin_addon = 8;// pin used for clock on addon board sht15
int dataPin_addon = 7; // pin used for data on addon board sht15
...
float sht_temperature_top_average = 0;
float sht_temperature_bottom_average = 0;
float sht_rh_top_average = 0;
float sht_rh_bottom_average = 0;
float ir_temp_average = 0;
float tryit [5];
...
void setup() {
  ...
  Wire.begin(I2C_MASTER,0x00,I2C_PINS_18_19,I2C_PULLUP_EXT,I2C_RATE_100);      // using alternative wire library
  ...
  while (Serial.peek() != '[' && Serial1.peek() != '[') {                      // wait till we see a "[" to start the JSON of JSONS
    ...
    if (choose.toInt() > 0 && choose.toInt() < 200) {                    // if it's 0 - 200 then go see full light of device testing
      ...
      case false:                                                                   // if it's not a number, then exit switch and check to see if it's a protocol
        break;
      case -1:                                                                    // print "MultispeQ Ready" to USB and Bluetooth
        delay(10);
        break;
...
//          Xcomp, Ycomp, Zcomp;
      ...
      case 1076:                                                                   // read IR sensor
        Serial.println(MLX90615_Read(0,0x5B));
        delay(500);
        break;
      case 1077:          // read IR sensor on main and add-on
          Serial.print(sht_temperature_top());
          Serial.print(",");
          Serial.print(sht_rh_top());
          Serial.print(",");
          Serial.print(sht_temperature_bottom());
          Serial.print(",");
          Serial.println(sht_rh_bottom());
        break;
        }
     }
  }        
...
//        get_calibration_userdef();
        ...
        g_average_forpar = 0;
        b_average_forpar = 0;
        sht_temperature_top_average = 0;
        sht_temperature_bottom_average = 0;
        sht_rh_top_average = 0;
        sht_rh_bottom_average = 0;
        ir_temp_average = 0;
      ...
      options for relative humidity, temperature, contactless temperature. light_intensity,co2
          ...
          for (int i=0;i<environmental.getLength();i++) {                                         // call environmental measurements
            ...
            if (environmental.getArray(i).getLong(1) == 0 \
            && (String) environmental.getArray(i).getString(0) == "sht_temp_rh") {
              sht_temperature_top();
              sht_temperature_bottom();
              sht_rh_top();
              sht_rh_bottom();
              if (x == averages-1) {
                Serial1.print("\"sht_temp_top\":");
                Serial.print("\"sht_temp_top\":");
                Serial1.print(sht_temperature_top_average);  
                Serial1.print(",");
                Serial.print(sht_temperature_top_average);  
                Serial.print(",");                
                Serial1.print("\"sht_rh_top\":");
                Serial.print("\"sht_rh_top\":");
                Serial1.print(sht_rh_top_average);  
                Serial1.print(",");
                Serial.print(sht_rh_top_average);  
                Serial.print(",");  
                Serial1.print("\"sht_temp_bottom\":");
                Serial.print("\"sht_temp_bottom\":");
                Serial1.print(sht_temperature_bottom_average);  
                Serial1.print(",");
                Serial.print(sht_temperature_bottom_average);  
                Serial.print(",");  
                Serial1.print("\"sht_rh_bottom\":");
                Serial.print("\"sht_rh_bottom\":");
                Serial1.print(sht_rh_bottom_average);  
                Serial1.print(",");
                Serial.print(sht_rh_bottom_average);  
                Serial.print(",");  
              }
            }
            if (environmental.getArray(i).getLong(1) == 0 \
            && (String) environmental.getArray(i).getString(0) == "ir_temp") {
              float ir_temp = MLX90615_Read(0,0x5B);
              if (x == averages-1) {
                Serial1.print("\"ir_temp\":");
                Serial.print("\"ir_temp\":");
                Serial1.print(ir_temp_average);  
                Serial1.print(",");
                Serial.print(ir_temp_average);  
                Serial.print(",");                
              }
            }
            if (environmental.getArray(i).getLong(1) == 0 \
            ...
            && (String) environmental.getArray(i).getString(0) == "compass") {                      // perform analog reads
              ...
              MAG3110_read(&Xcomp,&Ycomp,&Zcomp);
              if (x == averages-1) {
                Serial1.print("\"compass_x\":");
                Serial.print("\"compass_x\":");
                Serial.print(Xcomp);
                Serial1.print(",");
                Serial.print(",");                
                Serial1.print("\"compass_y\":");
                Serial.print("\"compass_y\":");
                Serial.print(Ycomp);
                Serial1.print(",");
                Serial.print(",");               
                Serial1.print("\"compass_z\":");
...
float MLX90615_Read(int TaTo,uint8_t address) {
  int rawData = MLX90615_getRawData(TaTo, address);
  double ir_temp = (rawData * 0.02);
  ir_temp -= 273.15;
  ir_temp_average += ir_temp / averages;
  return ir_temp;
}

static uint16_t MLX90615_getRawData(int TaTo,uint8_t address) {
  // Store the two relevant bytes of data for temperature
  byte dataLow = 0x00;
  byte dataHigh = 0x00;
...
uint16_t MLX90615_emissivity(double em,uint8_t address)   // .965 is a typical value
{
  // setup
...
double MLX90615_getEmissivity(uint8_t address) {
...
uint16_t MLX90615_getAddress(uint8_t address) {
  uint8_t dataLow = 0x00;
  uint8_t dataHigh = 0x00;
...
float sht_temperature_top() {
    int ack;  // track acknowledgment for errors
    int val;
    float temperature;
    // read the temperature and convert it to centigrades
    sendCommandSHT(temperatureCommand, 7, 8);
    waitForResultSHT(7);
    val = getData16SHT(7, 8);
    skipCrcSHT(7, 8);
    temperature = (float)val * 0.01 - 40;
    sht_temperature_top_average += temperature / averages;
    return temperature;
}

float sht_temperature_bottom() {
    int ack;  // track acknowledgment for errors
    int val;
    float temperature;
    // read the temperature and convert it to centigrades
    sendCommandSHT(temperatureCommand, 9, 10);
    waitForResultSHT(9);
    val = getData16SHT(9, 10);
    skipCrcSHT(9, 10);
    temperature = (float)val * 0.01 - 40;
    sht_temperature_bottom_average += temperature / averages;
    return temperature;
}
...
float sht_rh_bottom() {
    ...
    float humidity;
    // read the humidity
    sendCommandSHT(humidityCommand, 9, 10);
    waitForResultSHT(9);
    val = getData16SHT(9, 10);
    skipCrcSHT(9, 10);
    humidity = -4.0 + 0.0405 * val + -0.0000028 * val * val;
    delay(200); // wait for 200 milliseconds for next reading
    sht_rh_bottom_average += humidity / averages;
    return humidity;
}
