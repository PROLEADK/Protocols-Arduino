...
// changed it so that instead of adding additional actual analog reads, it just uses hardware averaging
// to change the number of analog reads, change the 
...
int saturatingcycleon = 5; //The cycle number in which the saturating light turns on (set = 0 for no saturating light)
int saturatingcycleoff = 10; //The cycle number in which the saturating light turns off
unsigned long pulselengthon = 30; // Pulse LED on length in uS (minimum = 5us based on a single ~4us analogRead - +5us for each additional analogRead measurement in the pulse).
unsigned long pulselengthoff = 99970; // Pulse LED off length in uS (minimum = 20us + any additional operations which you may want to call during that time).
...
int measurements = 4; // time required - if 1, 6us, if 2, 12us, if 3, 18us, etc.  number of measurements (analogreads) per pulse for hardware averaging.  
...
void setup() {
  ...
  analogReadAveraging(measurements); // set averaging to the number of desired measurements per pulse
...
//    caldata4 = analogRead(detector1);
    ...
    caldatatin[i] = caldata1; // data is stored as an array, size = calpulsecycles.  Each point in the array is the average of the data points from an ON cycle.  Averaging is done later.
  ...
  Serial.print("The last data point from the calibration: ");  
...
//    caldata4 = analogRead(detector1);
    ...
    start1=start1+pulselengthoff;
    digitalWriteFast(calibratinglight1, LOW);
    caldatatape[i] = caldata1; // data is stored as an array, size = calpulsecycles.  Each point in the array is the average of the data points from an ON cycle.  Averaging is done later.
  ...
  irtapevalue = (irtapevalue / calpulsecycles); //  Divide by the total number of samples to get the average reading during the calibration - NOTE! The divisor here needs to be equal to the number of analogReads performed above!!!! 
...
//    caldata4 = analogRead(detector1);
    ...
    start1=start1+pulselengthoff;
    digitalWriteFast(calibratinglight1, LOW); 
    caldatasample[i] = caldata1; // data is stored as an array, size = calpulsecycles.  Each point in the array is the average of the data points from an ON cycle.  Averaging is done later.
  ...
  irsamplevalue = (irsamplevalue / calpulsecycles); //  Divide by the total number of samples to get the average reading during the calibration - NOTE! The divisor here needs to be equal to the number of analogReads performed above!!!! 
  for (i=0;i<calpulsecycles;i++) { // Print the results!
...
//    data4 = analogRead(detector1);
  ...
  rebelsamplevalue = (rebelsamplevalue / pulsecycles); //  NOTE!  Divide by the total number of samples to get the average reading during the calibration - NOTE! The divisor here needs to be equal to the number of analogReads performed above!!!! 
...
void calculations() {
  ...
  for (i=0;i<pulsecycles;i++) { // Average each of the data points from the sample
    datasample[i] = 1000000*((reference*datasample[i])/(analogresolutionvalue));
  }
  for (i=0;i<pulsecycles;i++) { // Print the results!
