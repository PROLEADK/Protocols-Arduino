/*
THIS FILE TESTS IF IT'S POSSIBLE TO WRITE 80K FILE SIZES TO THE SD CARD (via saving the number 42 40k times)
...
int pulsecycles = 2500; // Number of times the "pulselengthon" and "pulselengthoff" cycle (on/off is 1 cycle) (maximum = 1000 until we can store the read data in an SD card or address the interrupts issue)
...
unsigned long pulselengthon = 5; // Pulse LED on length in uS (minimum = 5us based on a single ~4us analogRead - +5us for each additional analogRead measurement in the pulse).
...
char filename[13] = "STEST"; // Base filename used for data files and directories on the SD Card
...
void basicfluor() {
  ...
  for (i=0;i<pulsecycles;i++) {
    ...
    data1 = 424242; // NOTE! user needs to set the number of analogReads() per ON cycle - add more for more analogReads()
//    data2 = 4242424242;
//    data3 = 4242424242;
//    data4 = 4242424242;
    start1=start1+pulselengthon;
...
//    datasample[i] = (data1); // NOTE! ADJUST BASELINE MULTIPLIER BASED ON NUMBER OF DATA SAMPLED DURING MEASURING PULSE!  data is stored as an array, size = calpulsecycles.  Each point in the array is the average of the data points from an ON cycle.  Averaging is done later.
    file.println(data1); // (~10us) store data point on the SD Card
    file.println(data1); // (~10us) store data point on the SD Card
    file.println(data1); // (~10us) store data point on the SD Card
    file.println(data1); // (~10us) store data point on the SD Card
    while (micros()<start1) {
...
//    datasample[i] = ((reference*datasample[i])/(analogresolutionvalue*4)); // NOTE! The divisor here needs to be equal to the number of analogReads performed above, and the other number is the voltage reference (INTERNAL = 1.1, normal = 3.3)
//  }
  for (i=0;i<20;i++) { // Print the results!
    Serial.print(datasample[i]);
    Serial.println("");
  }
