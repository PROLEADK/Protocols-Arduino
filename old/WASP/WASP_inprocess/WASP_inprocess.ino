...
int repeatrun = 20; // # of times to repeat set1, set2, and set3 combined (limited based on dynamic array size of datasample[] - generally keep low at <5)
int measurements = 1; // FOR WASP LEAVE AT 1!  # of measurements per pulse (min 1 measurement per 6us pulselengthon)
int numberpulses1 = 30; // number of pulses in set 1
int numberpulses2 = 30; // number of pulses in set 2
...
float cyclelength1 = .1; // length of pulse cycle, set 1.  in seconds... minimum = pulselengthon + 7.5us
float cyclelength2 = .01; // length of pulse cycle, set 2.   in seconds... minimum = pulselengthon + 7.5us
float cyclelength3 = .001;// length of pulse cycle, set 3.  in seconds... minimum = pulselengthon + 7.5us
...
int* caldatasample;
int* datasample;
unsigned long datasampleaverage1;
unsigned long datasampleaverage2;
unsigned long datasampleaverage3;
unsigned long datasampleaverage4;
int* rebeldatasample;
int val = 0;
int cal = 0;
int cal2 = 0;
...
for (y=0;y<repeatrun;y++) {
  ...
  datasample = (int*)malloc((cyclelength1*numberpulses1+cyclelength2*numberpulses2+cyclelength3*numberpulses3)*sizeof(int)); // NOTE! change the number value based on the number of analogReads() per measurement.  create the array of proper size to save one value for all each ON/OFF cycle   
  ...
  delay(numberpulses1*cyclelength1*1000+10);
  z = 0; // reset counter z
  ...
  delay(numberpulses2*cyclelength2*1000+10);
  z = 0; // reset counter z
  ...
  // WAIT FOR TIMERS TO END (give it runlength plus a 10ms to be safe)
  delay(numberpulses3*cyclelength3*1000+10);
  z = 0; // reset counter z
...
void pulseoff() {
  ...
  digitalWriteFast(measuringlight1, LOW);
  datasampleaverage1 += data1;
  datasampleaverage2 += data2;
  datasampleaverage3 += data3;
  datasampleaverage4 += data4;
  datasample[z] = ((data1+data2+data3+data4)/4);
...
void stoptimers() { // Stop timers, close file and directory on SD card, free memory from datasample[], turn off lights, reset counter variables, move to next row in .csv file, 
  ...
  Serial.println(datasampleaverage1*3/(numberpulses1+numberpulses2+numberpulses3));
  Serial.println(datasampleaverage2*3/(numberpulses1+numberpulses2+numberpulses3));
  Serial.println(datasampleaverage3*3/(numberpulses1+numberpulses2+numberpulses3));
  Serial.println(datasampleaverage4*3/(numberpulses1+numberpulses2+numberpulses3));
  Serial.println("");
  datasampleaverage1 = 0;
  datasampleaverage2 = 0;
  datasampleaverage3 = 0;
  datasampleaverage4 = 0;
}
