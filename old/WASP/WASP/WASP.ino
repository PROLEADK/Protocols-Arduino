/*
DESCRIPTION:
  WASP stands for Weak Actinic Spectral Perterbation, and takes a measurements on top of the measuring pulse (~20us long) in increasingly dense pulses (start at 100ms distace between pulses
  ... then move to 10ms, then 1ms).  4 measurements are taken on the top of the 20us measuring pulse, and stored in an array datasample[].  The set of pulses (100, 10, 1) is repeated 10 times
  ... and can be averaged by the user in excel to get the final data.
...
// NOTES:
// SD card save, data array, serial print of data and jitter is all good.  see actual versus estimated run time in calculations() to confirm timing.  Confirmed 4 measurements in 25us fit ok!
// baseline adjustment is off but included, and baseline is measured using basicfluor() in the file standard_measurement_pmf_autocal3.  
...
int pulsecycles = 2; // # of times to repeat set1, set2, and set3 combined (limited based on dynamic array size of datasample[] - generally keep low at <5)
int measurements = 4; // number of measurements taken per pulse
int cycledelay = 100000; // Delay between cycles
int set1 = 5; // number of pulses in set 1
int timemeasurementsset1 = 25; // pulse width in set 1 in us
int timedelay1 = 99975; // delay between pulses in set 1 in us
int set2 = 5; // number of pulses in set 2
int timemeasurementsset2 = 25; // pulse width in set 2 in us
int timedelay2 = 9975; // delay between pulses in set 2 in us
int set3 = 5; // number of pulses in set 3
int timemeasurementsset3 = 25; // pulse width in set 3 in us
int timedelay3 = 975; // delay between pulses in set 3 in us
...
char filename[13] = "WASP"; // Base filename used for data files and directories on the SD Card
...
void WASP() {
  ...
  datasample = (int*)malloc((set1+set2+set3)*measurements*sizeof(int)); // Create the array of proper size to save one value for all each ON/OFF cycle
  noInterrupts();
 
start1orig = micros();
start1=micros();

for (z=0; z<pulsecycles; z++) {

  for (i=0;i<set1;i++) {
    digitalWriteFast(measuringlight1, HIGH); 
//    digitalWriteFast(calibratinglight1, HIGH);
    for (k=0;k<measurements;k++) {
      datasample[z*(set1+set2+set3)*measurements+i*measurements+k] = analogRead(detector1);
//      datasample[z*(set1+set2+set3)*measurements+i*measurements+k] = 1;
//      Serial.print(i);
//      Serial.print(",");      
//      Serial.println(k);
    }
    start1 = start1 + timemeasurementsset1;
    while (micros()<start1) {}  
    digitalWriteFast(measuringlight1, LOW); 
//    digitalWriteFast(calibratinglight1, LOW);
    start1 = start1 + timedelay1;
    while (micros()<start1) {}  
  }  

  for (i=set1;i<(set1+set2);i++) { 
    digitalWriteFast(measuringlight1, HIGH); 
//    digitalWriteFast(calibratinglight1, HIGH);
    for (k=0;k<measurements;k++) {
      datasample[z*(set1+set2+set3)*measurements+i*measurements+k] = analogRead(detector1);
//      datasample[z*(set1+set2+set3)*measurements+i*measurements+k] = 2;
//      Serial.print(i);
//      Serial.print(",");      
//      Serial.println(k);
    }
    start1 = start1 + timemeasurementsset2;
    while (micros()<start1) {}  
    digitalWriteFast(measuringlight1, LOW); 
//    digitalWriteFast(calibratinglight1, LOW);
    start1 = start1 + timedelay2;
    while (micros()<start1) {}  
  } 
  
  for (i=(set1+set2);i<(set1+set2+set3);i++) {
    digitalWriteFast(measuringlight1, HIGH); 
//    digitalWriteFast(calibratinglight1, HIGH);
    for (k=0;k<measurements;k++) {
      datasample[z*(set1+set2+set3)*measurements+i*measurements+k] = analogRead(detector1);
//      datasample[z*(set1+set2+set3)*measurements+i*measurements+k] = 3;
//      Serial.print(i);
//      Serial.print(",");      
//      Serial.println(k);
    }
    start1 = start1 + timemeasurementsset3;
    while (micros()<start1) {}  
    digitalWriteFast(measuringlight1, LOW); 
//    digitalWriteFast(calibratinglight1, LOW);
    start1 = start1 + timedelay3;
    while (micros()<start1) {}  
  } 
  start1 = start1 + cycledelay;
  while (micros()<start1) {}
}
end1 = micros();
...
//for (i=0;i<((set1+set2+set3)*measurements); i++) { // NOTE! COUNTER
//  datasample[i] = datasample[i] - baseline;
//}

Serial.println("SAMPLE RESULTS");
for (i=0;i<(pulsecycles*(set1+set2+set3)*measurements); i++) { // NOTE! COUNTER
  Serial.print(datasample[i]);
  Serial.print(",");
  if (i%((set1+set2+set3)*measurements) == ((set1+set2+set3)*measurements-1)) {  // NOTE! This will be relevent with the counter
    Serial.println("");
  }
}
...
void calculations() {
  ...
  Serial.println(pulsecycles*(cycledelay+set1*(timemeasurementsset1+timedelay1)+set2*(timemeasurementsset2+timedelay2)+set3*(timemeasurementsset3+timedelay3)));
