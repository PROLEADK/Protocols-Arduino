...
[{"protocol_name":"fluorescence","baselines":[1,1,1,1],"environmental":[["relative_humidity",1],["temperature",1]],"averages":1,"wait":0,"cal_true":0,"analog_averages":1,"act_light":20,"pulsesize":25,"pulsedistance":3000,"actintensity1":100,"actintensity2":100,"measintensity":3,"calintensity":255,"pulses":[50,50,50,50],"detectors":[[34],[34],[34],[34]],"measlights":[[15],[15],[15],[15]],"act":[2,1,2,2]}]
...
#define PWR_OFF 22           // teensy shutdown pin auto power off
...
// {firmware version: ......  sample: [[{data_raw:[3,4,2,3]},{data_raw:[...]...
  ...
  while (Serial.peek() != 91 && Serial1.peek() != 91) {                           // wait till we see a "[" to start the JSON of JSONS
    ...
    else if (Serial.peek() == 55 | Serial1.peek() == 55) {                      // run light tests
...
*/
  ...
  Serial1.read();                                       // flush the "["
                                                                                    // this is ugly but we couldn't get it to work with if statements without errors in the json2 string - COMBINE SO YOU DON'T REPEAT SO MUCH CODE BETWEEN BLUETOOTH AND SERIAL!
  ...
  switch (which_serial) {
    ...
    case 1:        
      ...
      case 2:
        ...
        Serial.println(Serial1.available());
#endif
          while (Serial1.peek()!=125) {                      // wait until you receive a "curly brackets" indicating the close of a json
            if (Serial1.available()>0) {
              w = Serial1.read();
              json2[number_of_protocols] += w;
            }
          }
          w = Serial1.read();                                            // catch the last 'curly brackets'
          json2[number_of_protocols] += w;
          Serial.println(json2[number_of_protocols]);      
          number_of_protocols++;
      #ifdef DEBUGSIMPLE                                                    // print out the incoming protocol JSONs
      #endif
          if (Serial1.peek() == 44) {                          // if there's a comma, flush it out before starting the next JSON
            Serial1.read();
          }
          else if (Serial1.peek() == 93) {                      // if it's a ], that means that's the end of the string of JSONs, so send up the end_flag
            end_flag = 1;
          }
      #ifdef DEBUGSIMPLE                                      // print the total number of protocols
          Serial.print("this is number_of_protocols     ");
...
int Light_Intensity_2(int var1) {
  if (var1 == 1 | var1 == 0 | var1 == 3) {
    word lux, r, g, b;
