          ...
          for (int z=0;z<total_pulses;z++) {                                            // cycle through all of the pulses from all cycles
            ...
            if (pulse < meas_array_size) {                                                                // if it's the first pulse of a cycle, then change act 1 and 2, alt1 and alt2 values as per array's set at beginning of the file
              if (pulse == 0) {
                ...
                act1_on = calculate_intensity(_act1_light,tcs_to_act,cycle,_light_intensity,tcs_to_act); // calculate the intensities for each light and what light should be on or off.
                _act2_light_prev = _act2_light;
                _act2_light = act2_lights.getLong(cycle);
                act2_on = calculate_intensity(_act2_light,tcs_to_act,cycle,_light_intensity,tcs_to_act);
                _alt1_light_prev = _alt1_light;
                _alt1_light = alt1_lights.getLong(cycle);
                alt1_on = calculate_intensity(_alt1_light,tcs_to_act,cycle,_light_intensity,tcs_to_act);
                _alt2_light_prev = _alt2_light;
                _alt2_light = alt2_lights.getLong(cycle);
                alt2_on = calculate_intensity(_alt2_light,tcs_to_act,cycle,_light_intensity,tcs_to_act);
              }
              calculate_intensity(_meas_light,tcs_to_act,cycle,_light_intensity,tcs_to_act);          // in addition, calculate the intensity of the current measuring light
...
int calculate_intensity(int _light,int tcs_on,int _cycle,float _light_intensity,int tcs_to_act) {  
//  #ifdef DEBUGSIMPLE
...
//#endif  
  ...
  if (_light == 2 | _light == 20) {                                                      // if it's a saturating light, and...
    if (act_intensities.getLong(_cycle) > 0) {                                            // if the actinic intensity is greater than zero then...
      ...
      on = 1;
      _tcs = (uE_to_intensity(_light,_light_intensity)*tcs_to_act)/100;
      Serial.println(_tcs);
      Serial.println(_tcs);
...
int calculate_intensity_background(int _light,int tcs_on,int _cycle,float _light_intensity,int _tcs, int _background_intensity) {    // calculate intensity of background light if it's set at a constant level by the user
#ifdef DEBUGSIMPLE
  Serial.println();
