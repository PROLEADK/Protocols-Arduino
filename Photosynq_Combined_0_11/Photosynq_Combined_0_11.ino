...
//    act [1] =            1;   
  ...
  case 3:                                    //////////////////// DIRK ////////////////////////
    ...
    pulsesize =           20;                  // us, length of pulse must be <100us
#ifdef NEWDET
    pulsesize =           150;                  // us, length of pulse must be <100us
#endif
...
*/
  ...
  if (protocol_number !=1 | protocol_number !=2) {
    start_json = micros();
    while (Serial.available()>0 | dif_json < 100000) {                      // wait until the serial cache is empty, or until it's been .1 seconds
    ...
    json = (char*)malloc((json2.length()+1)*sizeof(char));
    strcpy(json,json2.c_str());  
    json[json2.length()] = '\0';                                                    // Add closing character to char*
    Serial.println(json);
    ...
    {
        pulses[i] = pulses_.getLong(i);
        Serial.print(i);
        Serial.print(" ");
        Serial.println(pulses_.getLong(i));
    }
    
    for (int i = 0; i < (sizeof(pulses)/sizeof(int)); i++)
    {
        pulses[i] = pulses_.getLong(i);
        Serial.print(i);
        Serial.print(" ");
