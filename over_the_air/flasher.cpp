...
loop ()
{
  ...
  S_PORT.printf ("%x  %d\n", *(volatile unsigned int *) addr, ret);
...
// code to allow firmware update over a S_PORT port
...
// load new firmware over the S_PORT port and flash it
// format is an intel hex file followed by ":flash xx" where xx is the number of lines sent
// Max size is 1/2 of flash

// hint:  on Linux, exit the S_PORT console and do "dd if=blink.hex of=/dev/ttyACM0", then restart the S_PORT console and
// enter the ":flash xxx" command.

void
upgrade_firmware(void)
{
  S_PORT.printf("%s flash size = %dK in %dK sectors\n", FLASH_ID, FLASH_SIZE / 1024, FLASH_SECTOR_SIZE / 1024);
  ...
  if ((uint32_t)flash_word < FLASH_SIZE || (uint32_t)flash_erase_sector < FLASH_SIZE || (uint32_t)flash_move < FLASH_SIZE) {
    S_PORT.printf("routines not in ram\n");
    return;
  }
  ...
  while (addr > 0 && *((uint32_t *)addr) == 0xFFFFFFFF)
    addr -= 4;
  S_PORT.printf("current firmware 0:%x\n", addr + 4);

  if (addr > FLASH_SIZE / 2 - RESERVE_FLASH) {
    S_PORT.println("firmware is too large\n");
    return;
  }

  S_PORT.println("WARNING: this can ruin your device");
  S_PORT.println("waiting for intel hex lines");
...
// 2) Tie the send and receive lines of the S_PORT port together and it should recover.
...
void boot_check(void)
{
  ...
  S_PORT.print("@");
  delay(100);
  if (S_PORT.available() && S_PORT.read() == '@')
    upgrade_firmware();
...
// DANGER: if this is interrupted, the teensy is permanently destroyed
...
flash_move (uint32_t min_address, uint32_t max_address)
{
  ...
  for (address = min_address; address <= max_address; address += 4) {
    ...
    if ((address & (FLASH_SECTOR_SIZE - 1)) == 0) {   // new sector?
      flash_erase_sector(address, 54321);

      if (address == (0x40C & ~(FLASH_SECTOR_SIZE - 1)))  // critical sector
        flash_word(address, 0xFFFFFFFE);                  // fix it immediately
    }

    flash_word(address, *(uint32_t *)(address + FLASH_SIZE / 2));

  } // for

  // hint - can use LED here for debugging.  Or load the same program as is running.
...
flash_hex_line (const char *line)
{
  ...
  if (sscanf(line, ":flash %d", &lines) == 1) {
    if (lines == line_count && done) {
      S_PORT.printf("flash %x:%x begins...\n", min_address, max_address);
      delay(100);
      flash_move (min_address, max_address);
      // should never get here
    } else {
      S_PORT.printf ("bad line count %d vs %d or not done\n", lines, line_count);
      return -7;
    }
  } // if
  ...
  if (base_address + address + byte_count > FLASH_SIZE / 2 - RESERVE_FLASH) {
    S_PORT.print("address too large\n");
    error = 2;
    return -4;
  }
  ...
  switch (code)
  {
    ...
    case 4:
      ...
      return 0;
    default:
      S_PORT.printf ("err code = %d, line = %s\n", code, line);
      error = 3;
      return -3;
  }				// switch
  ...
  {
    S_PORT.printf ("can't flash %d bytes to %x\n", byte_count, address);
    error = 4;
    return -2;
  }
...
flash_erase_upper()
{
  ...
  for (address = FLASH_SIZE / 2; address < (FLASH_SIZE - RESERVE_FLASH); address += FLASH_SECTOR_SIZE) {
    if (!flash_sector_erased(address)) {
      S_PORT.printf("erase sector %x\n", address);
      if ((ret = flash_erase_sector(address, 0)) != 0)
        S_PORT.printf("flash erase error %d\n", ret);
    }
  } // for
} // flash_erase_upper()
...
flash_block (uint32_t address, uint32_t * bytes, int count)
{
  ...
  {
    S_PORT.print ("flash_block align error\n");
    return -1;
  }
  ...
  while (count > 0)
  {
    ...
    {
      S_PORT.printf ("flash_block write error %d\n", ret);
      return -2;
    }
    address += 4;
    ++bytes;
                ...
                unsigned int *code)
{
  ...
  *num = 0;
  ...
  if (!sscanf (ptr, "%04x", (unsigned int *)addr))
    return 0;
  ptr += 4;
  /* S_PORT.printf("Line: length=%d Addr=%d\n", len, *addr); */
