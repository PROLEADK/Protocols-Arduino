...
NEXT STEPS!!!!
get rid of the double brackets for measuring and detectors
now everything is single brackets
any light can occupy any space.  No limitation between actinic and measuring lights because now it's just on or off.  Only actinicintensity1 is used for actinics
allow act1,act2,alt1, and alt2 to have the light determined on each pulse
Questions - is there a flicker or change in light when the light is on, but a new value is pushed to the dac (try with the same dac value)

For example:
meas_light [15,16,11]
meas_intensity [0,0,45]
detector [34,35,34]
act_light [15,20,2]
act_intensity [140,0,3000]
... etc.
...
int _detector;                                                           // detector used in the current pulse
int act_state = 1;    
int act2_state = 1;
int act_int_state = 0;
...
//    delay(1000);
  ...
  for (y=0;y<measurements;y++) {                                                      // loop through the all measurements to create a measurement group
    ...
    for (int q = 0;q<number_of_protocols;q++) {                                               // loop through all of the protocols to create a measurement
      ...
      int act1_light =           hashTable.getLong("act1_light");                               // the Teensy pin # to associate with the actinic light  Must be pin 2, 20 (ie actinic pin only)
      int act2_light =          hashTable.getLong("act2_light");                                 // the Teensy pin # to associate with the actinic 2 light.  Must be pin 2, 20 (ie actinic pin only)
      int alt1_light =          hashTable.getLong("alt1_light");                             // the Teensy pin # to associate with the alternate light 2.  Must be pin 15,16,11,12 (ie measurement pin only)
      int alt2_light =          hashTable.getLong("alt2_light");                               // the Teensy pin # to associate with the alternate light 2  Must be pin 15,16,11,12 (ie measurement pin only)
...
// NOTE: for act, act2, alt1, and alt2, you can choose a measuring light.  If you do there are no preset states.  Select 2 for off, or 1 for on.
      ...
      int act_intensities =      hashTable.getLong("act_intensity");                         // write to input register of a DAC. channel 0 for low (actinic).  1 step = +3.69uE (271 == 1000uE, 135 == 500uE, 27 == 100uE)
      int meas_intensities =     hashTable.getLong("meas_intensity");                        // write to input register of a DAC. channel 3 measuring light.  0 (high) - 4095 (low).  2092 = 0.  From 2092 to zero, 1 step = +.2611uE
      int cal_intensities =      hashTable.getLong("cal_intensity");                        // write to input register of a DAC. channel 2 calibrating light.  0 (low) - 4095 (high).
      ...
      for (i=0;i<pulses.getLength();i++) {
        size_of_data_raw += pulses.getLong(i);
      }
...
[{"averages":1,"wait":0,"cal_true":0,"analog_averages":1,"act_light":20,"pulsesize":10,"pulsedistance":10000,"actintensity1":27,"actintensity2":1370,"measintensity":147,"calintensity":4095,"pulses":[50,50,50,50],"detectors":[[34],[34],[34],[34]],"measlights":[[15],[15],[15],[15]],"act":[2,1,2,2]}]
*/
      ...
      if (tcs_to_act > 0) {                                                           // if you're setting actinic intensity based on outside light as per the tcs light sensor.  The deep red actinic produces 103 uE per pwm step.
        actintensity1 = (lux_to_uE(lux_average)*tcs_to_act)/(3.69*100);
...
//      Serial.println(end1-start1);
      dac.analogWrite(1,actintensity1);                                                       // write to input register of a DAC. channel 1, for low (actinic).  0 (low) - 4095 (high).  1 step = +3.69uE
...
//      analogWrite(ACTINICLIGHT_INTENSITY1, actintensity1);                      // set intensities for low (actinic)
      ...
      dac.analogWrite(2,measintensity);                                                       // write to input register of a DAC. channel 2, calibrating light.  0 (low) - 4095 (high).  1 step = +3.654uE  
      dac.analogWrite(3,calintensity);                                                       // write to input register of a DAC. channel 3 measuring light.  0 (high) - 4095 (low).  2092 = 0.  From 2092 to zero, 1 step = +.2611uE
...
right after detecting the end of the on/off cycle, then:

first, shift all the old variables to new ones from the json, and save old ones as _last
Turn off old lights IMMEDIATELY FOLLOWED BY
turn on new lights followed by
change dac setting
REPEAT FOR EACH LIGHT

for example:

_meas_light_last = _meas_light;
_meas_light = pull from json;
_meas_intensity = pull from json;

... DO FOR ALL ACT,ALT,detector, etc.

digitalWriteFast(_meas_light_last,LOW);
digitalWriteFast(_meas_light,HIGH);
dac.analogWrite(meas_pwm,_meas_intensity);
... DO FOR ALL ACT,ALT,detector, etc.
...
*/
          ...
          _meas_light = meas_lights.getLong(cycle);                                                                  // move to next measurement light

          _detector = detectors.getLong(cycle);                                        // move to next detector
                    
          _act_intensity1 = act_intensity1.getLong(cycle);  // pull new intensities from the JSON
          _meas_intensity = meas_intensity.getLong(cycle);
          _cal_intensity = cal_intensity.getLong(cycle);
          ...
          if (pulse == 0) {                                                                // if it's the first pulse of a cycle, then set actinic 2 off or on.  The preset position is set by actinic 1, so values are 2 = off, 1 or 0 = on.
            if (act2.getLong(cycle) == 2) {
              act2_state = 0;
            }
            else {
              act2_state = 1;
          ...
          digitalWriteFast(ACTINICLIGHT_INTENSITY_SWITCH,act_int_state);                        // flip the switch between actinic states if necessary
          ...
          data1 = analogRead(_detector);                                              // save the detector reading as data1, and subtract of the baseline (if there is no baseline then baseline is automatically set = 0)     
#ifdef DEBUGSIMPLE
          ...
          for (i=0;i<baselines.getLength();i++) {                                      // check the current light and see if a baseline should be applied to it.
            if (baselines.getLong(i) == 1) {
              if (baseline_lights[i] == _meas_light.getLong(cycle)) {
...
void pulse1() {		                                                        // interrupt service routine which turns the measuring light on
  ...
  data1 = analogRead(_detector);                                              // save the detector reading as data1
  on=1;
#ifdef DEBUG
