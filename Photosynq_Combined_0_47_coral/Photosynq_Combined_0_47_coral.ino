...
uint16_t userdef51[256] = {};  // 512 bytes
uint16_t userdef52[256] = {};  // 512 bytes
...
const char* bt_response = "OKOKlinvorV1.8OKsetPINOKsetnameOK115200"; // Expected response from bt module after programming is done.
float freqtimer0;
float freqtimer1;
float freqtimer2;
...
void loop() {
  ...
  int _alt1_light;
  int _alt2_light;
  int _act1_light_prev = 0;
  int _act2_light_prev = 0;
  int _alt1_light_prev = 0;
  int _alt2_light_prev = 0;
    ...
    if (choose.toInt() > 0 && choose.toInt() < 200) {                    // if it's 0 - 200 then go see full light of device testing
      ...
      switch (choose.toInt()) {
        ...
        case 1033:                                                                   // save user defined value to EEPROM for how many ms to delay before batteries shutoff (pwr_off_ms)
          ...
          Serial.print("{");
          Serial1.print("{");
          EEPROM_readAnything(1440, pwr_off_ms);                              // reads two floats
        ...
        case 1076:                                                                   // save user defined value to EEPROM
          Serial.println(MLX90614_Read(0));
          break;
        case 1077:                                                                   // save user defined value to EEPROM
          readSpectrometer(100, 35, 35, false);
          print_data();
          break;
        case 1078:                                                                   // save user defined value to EEPROM
          add_userdef2(24, 256, sizeof(uint16_t));
          break;
        case 1079:                                                                   // save user defined value to EEPROM
          add_userdef2(536, 256, sizeof(uint16_t));
          break;
      }
    }
  }
        ...
        */
        ...
        for (int x = 0; x < averages; x++) {                                                 // Repeat the protocol this many times
          ...
          */
          ...
          Serial.println(lux_to_uE(lux_local));
          Serial.print("ue to intensity result:  ");
          Serial.println(uE_to_intensity(act_background_light, lux_to_uE(lux_local))); // NOTE: wait turn flip DAC switch until later.
#endif
...
skipPart:                                                                             // skip to the end of the protocol
        ...
        if (spec_on == 1) {                                                                    // if the spec is being used, then read it and print data_raw as spec values.  Otherwise, print data_raw as multispeq detector values as per normal
          ...
          for (int i = 0; i < SPEC_CHANNELS; i++)
          {
            Serial.print(spec_data_average[i] / averages);
            Serial1.print(spec_data_average[i] / averages);
...
// recall everything from eeprom, optionally print it out
...
void call_print_calibration (int _print) {
  ...
  EEPROM_readAnything(24, userdef51);                                // 256 values, 512 bytes
  //  EEPROM_readAnything(1448,userdef52);                           // 256 values
  EEPROM_readAnything(536, userdef52);                               // 256 values, 512 bytes ending at 1048
...
// read in and store n uint16s to a specified eeprom location

// bytes is the size of each element (always 2?)

void add_userdef2(int location, int num_of_objects, int bytes) {

  uint16_t userdef[num_of_objects];

  //    call_print_calibration(1);
  for (int i = 0; i < num_of_objects; i++) {                  // loop "num_of_objects" times for objects which are "bytes" size for each object
    userdef[i] = (uint16_t) user_enter_dbl(60000);
    //      Serial.println(userdef);
    //      Serial1.println(userdef);
  }
  
  // now write
  for (int i = 0; i < num_of_objects; i++) {         // note, this can take awhile
    EEPROM_writeAnything(location + i * bytes, userdef[i]);
...
// power_off == 0 enables a range check
...
void add_userdef(int location, int num_of_objects, int bytes, int power_off) {
  ...
  //    call_print_calibration(1);
  for (int i = 0; i < num_of_objects; i++) {                  // loop "num_of_objects" times for objects which are "bytes" size for each object
    userdef = (float) user_enter_dbl(60000);

    if (power_off == 0) {        // range limits
      if (userdef == 0)
        userdef = 9999999;
      else if (userdef < 5000)       // in case pwr_off_ms is less than 5 seconds, set it to 2 minutes by default, otherwise leave it as whatever the user defined in EEPROM
        userdef = 120000;
    }
...
void configure_bluetooth () {
  ...
  const char* testMsg =   "PhotosynQ changing bluetooth name!!"; //
  int x;
...
void print_data()
{
  ...
  for (int i = 0; i < SPEC_CHANNELS; i++)
  {
    Serial.print(spec_data[i]);
    Serial1.print(spec_data[i]);
    if (i != SPEC_CHANNELS - 1) {               // if it's the last one in printed array, don't print comma
      Serial.print(',');
      Serial1.print(',');
    }
